...
//            return appointmentRepository.findByDoctorId(currentUser.getId(), pageable)
...
//        User currentUser = getCurrentUser();
//
//        if (!isAdmin(currentUser) && !currentUser.getId().equals(doctorId)) {
...
//
//        User currentUser = getCurrentUser();
//        if (!isDoctor(currentUser) || !appointment.getDoctor().getId().equals(currentUser.getId())) {
...
//                (isDoctor(currentUser) && appointment.getDoctor().getId().equals(currentUser.getId())) ||
//                isAdmin(currentUser);
//
//        if (!canCancel) {
...
//                        ? appointment.getDoctor().getUser().getName() : null)  // ✅ FIXED
...
public class AppointmentService {
    ...
    public List<AppointmentDto> getDoctorUpcomingAppointments(Long doctorId) {
        ...
        // Only the doctor themselves or admin can access doctor's appointments
        if (!isAdmin(currentUser) && !currentUser.getId().equals(doctorId)) {
            throw new InvalidAppointmentException("Access denied");
        }
